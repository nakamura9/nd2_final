...
void run_color_auto(){
  ...
  get_color();
  int red_difference = COLOR_SET_POINT.red - red_channel;
  int green_difference = COLOR_SET_POINT.green - green_channel;
  int blue_difference = COLOR_SET_POINT.blue - blue_channel;
  if(red_difference > 5 || green_difference > 5 || blue_difference > 5){
...
void set_register_positions(byte message[5]){
    int register_one = map(message[0], 0, 255, 0, 180);
    int register_two = map(message[1], 0, 255, 0, 180);
    //release clutches
    digitalWrite(CLUTCH, HIGH);
